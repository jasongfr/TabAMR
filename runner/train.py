...
class BaseTrainer:
    ...
    def write_log(self):
        ...
        print("-----------------------global_step: {} -------------------------------- ".format(
            self.metric['global_steps']))
        print('smooth_loss: {}'.format(self.metric['smooth_loss']))
        with open(r'log.txt', 'a+', encoding='utf-8') as file:
            file.write("-----------------------global_step: {} -------------------------------- ".format(self.metric['global_steps'])+'\n')
            file.write('smooth_loss: {}'.format(self.metric['smooth_loss'])+'\n')
            file.write('lr: {}'.format(self.scheduler.get_last_lr()[0])+'\n')
...
class Trainer(BaseTrainer):
    ...
    def convert_batch_to_inputs(self, batch):
        inputs = {
            ...
            'enc_amr_ids':              batch[16].to(self.cfg.device),
            'amr_len_prompt_ids':       batch[17],
            'amr_start_end':            batch[18]
        }
