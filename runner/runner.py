...
class BaseRunner:
    def __init__(
        ...
        self.cfg = cfg
        self.model = model
        self.dev_samples, self.test_samples = data_samples
        self.dev_features, self.test_features = data_features
        self.dev_loader, self.test_loader = data_loaders
...
class Runner(BaseRunner):
    ...
    def eval_and_update(self, global_step):
        ...
        print('current best dev-f1 score: {}'.format(self.metric["best_dev_f1"]))
        print('current related test-f1 score: {}'.format(self.metric["related_test_f1"]))
        with open(r'log.txt', 'a+', encoding='utf-8') as file:
            file.write('current best dev-f1 score: {}'.format(self.metric["best_dev_f1"])+'\n')
            file.write('current related test-f1 score: {}'.format(self.metric["related_test_f1"])+'\n')
