...
class Seq2Table(RobertaPreTrainedModel):
    def __init__(self, config, decode_layer_start=17, num_prompt_pos=0, num_event_embed=0):
        ...
        if self.num_prompt_pos > 0:
            self.event_type_embed = nn.Embedding(num_prompt_pos, config.hidden_size,
                                                 _weight=torch.zeros(num_prompt_pos, config.hidden_size), padding_idx=0)
        self.num_event_embed = num_event_embed
        if self.num_event_embed > 0:
            self.event_embed = nn.Embedding(num_event_embed, config.hidden_size,
                                            _weight=torch.zeros(num_event_embed, config.hidden_size), padding_idx=0)
    ...
    def forward(
        ...
        amr_attention_mask = torch.zeros_like(enc_amr_ids)
        for i, len_amr_ids in enumerate(amr_len_prompt_ids):
            amr_attention_mask[i, :len_amr_ids] = 1

        dec_amr_embeds = self.roberta(
            input_ids=enc_amr_ids,
            attention_mask=amr_attention_mask,
            cross_attention=False
        ).last_hidden_state

        for i,start_end in enumerate(amr_start_end):
            dec_table_embeds[i,start_end[0]:start_end[1]] = dec_amr_embeds[i, 0:amr_len_prompt_ids[i]]
        ...
        for i, num_event_types in enumerate(list_num_event_types):
            ...
            for j, len_prompt_ids in enumerate(list_len_prompt_ids_):
                dec_table_embeds[i, cum_len: cum_len + len_prompt_ids] = dec_prompt_embeds[cusor,
                                                                         :len_prompt_ids]
                if self.num_prompt_pos > 0:
        ...
        for i, (encoder_output, trigger_index, list_arg_slots_) in \
            ...
            for j, (trigger_start, trigger_end) in enumerate(trigger_index):
                ...
                dec_table_embeds[
                    i, dec_trigger_index_] = trigger_embed
                if self.num_event_embed > 0:
        ...
        for i, (context_output, decoder_table_output, list_arg_slots_, list_roles_, old_tok_to_new_tok_index) in \
                enumerate(
                    zip(context_outputs, decoder_table_outputs, list_arg_slots, list_roles, old_tok_to_new_tok_indexs)):
            batch_loss = list()
            cnt = 0
            ...
            for j, (arg_slots, roles) in enumerate(zip(list_arg_slots_, list_roles_)):
                ...
                for (slots, arg_role) in zip(arg_slots, roles):
                    ...
                    for slot in slots:
                        query_sub = decoder_table_output[slot].unsqueeze(0)  # [1, H]
