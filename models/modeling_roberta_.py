...
class RobertaEncoder(nn.Module):
    def __init__(self, config, decode_layer_start):
        super().__init__()
        self.config = config
        self.decode_layer_start = decode_layer_start
        encode_layers = [RobertaLayer(config) for _ in range(decode_layer_start)]
        decode_layers = [RobertaLayer(config, cross_attention=True) for _ in range(config.num_hidden_layers-decode_layer_start)]
...
class RobertaModel_(RobertaPreTrainedModel):
    ...
    def __init__(self, config, decode_layer_start, add_pooling_layer=False):
        super().__init__(config)
        self.config = config

        self.embeddings = RobertaEmbeddings(config)
        self.encoder = RobertaEncoder(config, decode_layer_start)
