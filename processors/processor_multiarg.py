...
class InputFeatures(object):
    ...
    def __init__(self, example_id, feature_id, list_event_type, list_event_trigger,
                 ...
                 list_dec_prompt_ids, enc_amr_ids, amr_start_end
                 ):
        ...
        self.enc_amr_ids = enc_amr_ids
        self.amr_start_end = amr_start_end
...
class ArgumentExtractionDataset(Dataset):
    ...
    def collate_fn(batch):
        ...
        enc_amr_ids = [f.enc_amr_ids for f in batch]
        amr_len_prompt_ids = [len(x) for x in enc_amr_ids]
        max_amr_len = max(amr_len_prompt_ids)
        enc_amr_ids = torch.LongTensor([ids + [1 for _ in range(max_amr_len - len(ids))]
                                                for ids in enc_amr_ids])
        amr_len_prompt_ids = torch.LongTensor(amr_len_prompt_ids)
        amr_start_end = [f.amr_start_end for f in batch]
        ...
        return enc_input_ids, enc_mask_ids, \
            ...
            trigger_enc_token_index, \
            list_arg_2_prompt_slots, cum_event_nums_per_type, \
            list_dec_prompt_ids, list_len_prompt_ids, enc_amr_ids, amr_len_prompt_ids, amr_start_end
...
class MultiargProcessor(DSET_processor):
    ...
    def convert_examples_to_features(self, examples, role_name_mapping=None):
        ...
        for example in examples:
            ...
            sep_index = raw_context.index('<sep>')
            context = raw_context[:sep_index]
            amr = raw_context[sep_index + 1:]
            ...
            if len(enc_input_ids) > self.args.max_enc_seq_length:
                print(self.args.max_enc_seq_length)
            ...
            len_dec_table_before = len(dec_table_ids)
            amr_start_end = []
            amr_start_end.append(len_dec_table_before)

            lineared_amr = deepcopy(amr)
            lineared_amr = " ".join(lineared_amr)
            self.tokenizer.add_tokens(['spatial', 'temporal', 'means', 'Arg', 'prepositions', '<node>', '(', ')', 'Arg-of'])
            enc_amr = self.tokenizer(lineared_amr, add_special_tokens=True)
            enc_amr_ids, enc_amr_mask_ids = enc_amr["input_ids"], enc_amr["attention_mask"]
            if self.args.max_dec_seq_length - len_dec_table_before < len(enc_amr_ids):
                shengxia = self.args.max_dec_seq_length - len_dec_table_before
                enc_amr_ids = enc_amr_ids[:shengxia]
                enc_amr_mask_ids = enc_amr_mask_ids[:shengxia]

            dec_table_ids += enc_amr_ids
            len_dec_table_after = len(dec_table_ids)

            amr_start_end.append(len_dec_table_after)
            ...
            for i, trigger_pos in enumerate(list_trigger_pos):
                ...
                dec_table_attention_mask[trigger_pos + 1: trigger_pos + 1 + num_arg_slots,
                trigger_pos + 1: trigger_pos + 1 + num_arg_slots] = 1
            dec_table_attention_mask[len_dec_table_before:len_dec_table_after, dec_prompt_lens:len_dec_table_before] = 1
            dec_table_attention_mask[dec_prompt_lens:len_dec_table_after, len_dec_table_before:len_dec_table_after] = 1
            ...
            feature = InputFeatures(example_id, feature_idx, list_event_type, triggers,
                                    ...
                                    dec_table_ids, dec_table_attention_mask, dec_prompt_lens,
                                    list_arg_slots, list_target_info,
                                    old_tok_to_new_tok_index, context, list_roles,
                                    list_arg_2_prompt_slots, cum_event_nums_per_type,
                                    list_dec_prompt_ids, enc_amr_ids, amr_start_end
                                    )
